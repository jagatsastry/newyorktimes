...
#        print ent1,ent2
       ...
       # w2 = [x[0] for x in reversed(sorted(mp[ent2], key=lambda x: x[1]))]
        ...
        w1 = w1[0:min(100, min(len(w1), len(w2)))]
        w2 = w2[0:len(w1)]
...
#        print "t3: ",t3 - t2
        for row, column in indices:
            ...
            if value <= 0:
                ...
                wordfileEntries.append("%3d\t%3d\t%3d\t%3d\t%2.3f\n" % (ent1, w1[row], ent2, w2[column], -1 * value))
...
outfile = open("wsim_100_words/" + root + ".graph", "w")
wordfile = open("wsim_100_words/" + root + ".wscore", "w")
